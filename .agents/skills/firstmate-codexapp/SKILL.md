...
description: >-
  Agent-only playbook for coordinating visible Codex Desktop threads alongside Number One without pretending they are a selectable shell backend.
  Use before creating, reading, steering, archiving, debugging, or reviewing a Codex App visible thread for Number One work, and before responding to requests to make Codex App native to Number One.
...
Use this playbook when Number One work needs a visible Codex Desktop thread.
...
Codex Desktop visible threads are companion host-tool workflows, not a selectable Number One backend.
...
If local helper scripts exist for Codex App work, use only helpers explicitly provided by the operator or maintained by Number One.
...
Use paused: only for a deliberate known external wait that should be rechecked later, never for a blocker that needs Number One to act.
...
Do not claim it is a complete Number One backend.
...
For Number One reconciliation, prefer concrete evidence:
...
Avoid repeating long transcripts into Number One docs or PR bodies.
...
If there is a real Number One task record, leave teardown decisions to the normal Number One task flow instead of this skill.
