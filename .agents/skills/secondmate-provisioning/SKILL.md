...
description: >-
  Agent-only reference for persistent second officer setup and retirement.
  Use when creating, seeding, validating, launching, recovering, handing backlog to, pushing inherited local material into, or retiring a second officer home, or when editing data/secondmates.md.
  Covers home leases, transactional seeding, project clone restrictions, second officer harness pins, inherited local-material push, idle charter, handoff helper, and teardown safety.
...
Use this reference before creating, seeding, validating, launching, handing backlog to, recovering, pushing inherited local material into, or retiring a persistent second officer, and before editing `data/secondmates.md`.

Keep the always-inline routing rules in `AGENTS.md` authoritative: route by natural-language `scope:`, local-only projects stay with the main Number One, and second officers are idle by default.

## Routing table

`data/secondmates.md` has one parser-compatible line per persistent second officer:
...
Scaffold a second officer charter with:
...
Pass `--no-projects` instead of a project list to scaffold a project-less charter for a domain whose subject is the Number One repo itself, whose home is a Number One worktree and whose crews take pooled worktrees of the same repo.
...
`-` durably leases a fresh Number One worktree via `treehouse get --lease` under the second officer id.
...
`bin/fm-home-seed.sh` copies the charter into the second officer home as `data/charter.md`.
...
`bin/fm-spawn.sh --secondmate` launches it through the second officer harness path, resolving `config/secondmate-harness` -> `config/crew-harness` -> the primary's own harness unless an explicit per-spawn harness override is passed.

`config/secondmate-harness` may also pin a concrete model and effort for the second officer agent, in the SAME file rather than a new one: the format is a single whitespace-separated line `<harness> [<model>] [<effort>]`, with only the first non-empty, non-comment line parsed.
...
For a `--secondmate` spawn, `bin/fm-spawn.sh` populates `MODEL`/`EFFORT` from those tokens only when the harness itself came from the second officer config path for that spawn.
...
The locked session-start bootstrap sweep runs the same guarded fast-forward for every live second officer home, discovered from `state/<id>.meta` records with `kind=secondmate` (`data/secondmates.md` only backfills `home=` for older records).
That no-fetch path is a purely local fast-forward of tracked files, never an origin fetch, and it never touches the gitignored operational dirs, so a second officer's backlog, projects, and in-flight work are never disturbed; a linked worktree advances immediately, while a standalone clone that lacks the target receives Number One updates through `/updatefirstmate`'s origin refresh.
...
Propagation failures warn without blocking second officer launch or session-start continuation, and the destination keeps whatever safely validated state the helper left behind.
Inheritance copies the literal `config/crew-harness` file, so a second officer's own crew members use the primary's crew member harness only when it names a concrete adapter such as `codex`; an unset or `default` value has nothing concrete to inherit, and the second officer's own crew members fall back to the second officer's own or detected harness instead.
Inherited `config/backend` becomes that second officer home's local runtime-backend default for future spawns only; it never retargets, rewrites, migrates, stops, or restarts an already-live worker endpoint.
A present primary value always converges byte-exact into validated second officer homes, and primary absence removes the destination so those homes keep runtime auto-detection.
...
`config/secondmate-harness` is not inherited because it is only the primary's knob for launching second officer agents.
`data/captain-shared.md` is main-authoritative in the primary home and read-only in second officer homes.
Its primary file header must state that the file is main-authoritative, read-only in second officer homes, must not be edited there, and that new captain-preference discoveries are routed to the main Number One through marked status or a document pointer.
Every propagation point converges the second officer copy to the primary bytes; when the primary file is absent, any existing second officer copy is quarantined and removed so absence converges too.
...
Between propagation runs, the second officer copy is filesystem read-only; the helper may make its owned destination writable only around a guarded update and restores read-only mode on success, unchanged bytes, and recoverable failure paths.
Before replacing divergent second officer bytes, the helper hash-compares source and destination, quarantines the secondmate-local version to a collision-safe private dated sibling file, and emits a `SECONDMATE_SYNC:` diagnostic naming the home and quarantine artifact.
Never copy any second officer `data/captain-shared.md` back into the primary.
...
Keep every `data/learnings.md` fully local by captain decision; route fleet-general machinery facts into tracked documentation through the normal Number One repo path rather than inventing shared learnings propagation.
...
A separate, literal-content config reread is required whenever inherited `config/*` material changes under an already-running second officer.
...
A newly launched or relaunched second officer already reads its files at launch, so its pending config-reread generations are discarded or quarantined after cleanup failure and it needs no redundant live-agent config nudge unless propagation changes files after launch.
...
For already-live second officers, use `bin/fm-config-push.sh` to push a mid-session inherited local-material change without running the tracked-file fast-forward.
...
Second officer project lists may include `no-mistakes` and `direct-PR` projects only.
`local-only` projects stay with the main Number One.
...
When a second officer is created for a domain, existing main-backlog items that fall under its scope should become its work instead of staying stranded in the main backlog.
Scope-matching is Number One's judgment against the second officer's natural-language scope, not a keyword rule.
...
After seeding, run this handoff for the new second officer's in-scope queued items.
The helper resolves and validates the second officer home from `data/secondmates.md`, then delegates the item move to `tasks-axi mv` (the single owner of the backlog format), which moves each named item - and a whole connected set, blocker plus dependents, atomically - from the main `data/backlog.md` into the second officer home's `data/backlog.md`.
This delegated route remains required when `config/backlog-backend=manual`, which controls only routine Number One backlog edits.
...
It is idempotent; an item already in the second officer backlog is skipped.
It refuses any destination that is not a genuine seeded Number One home with safe operational directories and a matching `.fm-secondmate-home` marker, so a move can never land in a project.
Do not hand off `local-only` items.

## Recovery

For `kind=secondmate` meta with no window, treat the second officer as a dead persistent direct report and respawn it with:
...
If meta is missing but `data/secondmates.md` still registers the second officer, respawn from the registry entry and its persistent on-disk home.
Respawn re-resolves the second officer harness from current config, uses the same guarded pre-launch sync, and re-propagates inherited local material, so recovered second officers converge inherited config items and shared captain preferences whenever their home validates; tracked-file sync remains guarded separately.
If the second officer is already running and only inherited local material changed, prefer `bin/fm-config-push.sh` over respawning.

Do not reconstruct a second officer's whole tree from the main home.
The main Number One reconciles only direct reports.
Each second officer is a Number One in its own home, so it runs recovery on startup and reconciles its own crew members.
A second officer's recovery reconciles only work that is already its own and then idles.
It never initiates a survey or audit during recovery.

## Retirement and teardown

A second officer is persistent by default.
An empty queue is healthy and does not trigger teardown.
Run `bin/fm-teardown.sh <id>` for `kind=secondmate` only when the captain or main Number One explicitly decides to retire that persistent second officer.

The safety check is the second officer's own home.
Teardown refuses while its `state/*.meta` contains in-flight work.
When safe, teardown kills the direct tmux window, removes the `data/secondmates.md` route, clears the main home metadata, and removes the retired second officer home.
...
It kills child windows, discards child work and state inside the second officer home, removes the route, releases the lease, and removes the retired second officer home.
