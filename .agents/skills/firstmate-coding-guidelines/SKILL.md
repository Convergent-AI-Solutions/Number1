...
description: >-
  Agent-only reference for changing Number One's shared, tracked material per AGENTS.md section 1.
  Use before editing any of that material, whether working as Number One directly or as a crew member briefed on a firstmate-repo task.
...
Load this before changing Number One's shared, tracked material, as defined by `AGENTS.md` section 1.
...
1. Does the Number One AGENT need this on every session or every turn to operate?
...
Briefs for tasks that touch Number One's own tracked material should tell the crew member to load this skill.
`bin/fm-brief.sh`'s `REPO` argument is a caller-supplied string with no reliable signal that it names Number One's own repo, unlike a project registered in `data/projects.md`, so there is no clean point inside the scaffold to detect this case automatically.
Number One adds this skill's load instruction to firstmate-repo briefs by hand instead.
