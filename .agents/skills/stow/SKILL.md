...
Sweep this session for durable knowledge that only exists in conversation right now, and write it to the disk locations Number One already prints in the next session-start context digest.
...
1. **Sweep the session for uncaptured durable knowledge.**
   ...
   - Operational learnings: fleet-local facts and gotchas discovered while operating Number One (a script's sharp edge, a harness quirk, a recurring false alarm and its real cause).
...
3. **Write within Number One's existing write boundaries.**
   This skill does not grant any new write permission; it only prompts Number One to use the boundaries that already exist (AGENTS.md section 1):
   ...
   - Project-intrinsic knowledge: never hand-write a project's `AGENTS.md`.
     Route it through a normal ship task so a crew member records it via `bin/fm-ensure-agents-md.sh` and commits it through that project's delivery pipeline, exactly as section 6 describes.
     If the fleet is live, delegate this to a crew member rather than doing it inline.
   - Knowledge generalizable to every Number One user: this repo's own `AGENTS.md` (or other shared, tracked material), shipped through the normal branch -> no-mistakes -> PR -> captain-merge pipeline for this repo (section 1), never hand-committed straight to `main`.
...
5. **Report to the captain.**
   ...
   If something could not be captured yet (for example, project-intrinsic knowledge waiting on a crew member to land it), say so explicitly rather than reporting the session fully safe.
...
Writing learnings into either `.agents/skills/` or public `skills/` would still risk mixing fleet-local material with shared Number One behavior or standalone installer-facing behavior.
Until a human deliberately scopes a skill change as Number One repo work, route generalizable knowledge to the shared `AGENTS.md` (or other shared, tracked material) via the pipeline, and fleet-local knowledge to `data/`, never to a skill.
