...
It is off by default, and the last `/calm` choice persists for the effective Number One home across Pi session starts and resumes.

While Calm is active, Pi's built-in `Working...` activity remains visible and no separate Calm status row is added.
Calm hides collapsed thinking labels, the shells for Pi's seven built-in tools, the `fm_watch_arm_pi` tool shell, and canonically classified Number One operational user rows.
...
Every hidden Number One input remains available to the model and in serialized session data and exported artifacts.
