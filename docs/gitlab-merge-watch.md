...
Its JSON would need a JSON processor, and `jq` is not one of Number One's common tools, so the state is read from glab's own field output instead.
Only an exact `merged` wakes Number One, so a changed output format produces no wake rather than a false merge.
...
`gh` exposes the head commit as a selectable field, while plain `glab` exposes it only inside its JSON output, which would need a JSON processor Number One does not require.
