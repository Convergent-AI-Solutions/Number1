...
This document is the authoritative human-readable contract for the guard that stops a Number One primary from delegating work outside the fleet.
...
On 2026-07-22 a Number One primary ran four workers through Claude Code's built-in subagent tool instead of `bin/fm-spawn.sh`.
...
- When the primary session restarted, two of those workers died mid-flight and their work was lost.
  A real crew member lives in its own backend session with durable state and survives a primary restart.
...
- `PLAN_ONLY_TOOLS`: the exact names `taskcreate` and `taskupdate` are allowed.
  ...
  That list has no executor: it spawns no agent, allocates no worktree, registers no schedule, and starts nothing that could outlive the session or escape a Number One guard.
  So it is not the "work, agent, schedule, or isolated workspace that Number One would not know about" the guard exists to stop, and the stem match on `task` is a false positive rather than a policy.
...
- A tracked `.claude/settings.json` propagates into linked worktrees and disarms legitimate crew members.
...
The shipped hook fires only in a genuine Number One primary home, using the shared predicate `fm_primary_scope_matches` from `bin/fm-primary-scope-lib.sh`.
...
A marked second officer home is in scope on purpose: it operates its own fleet and must dispatch through it for the same durability reasons.

A crew member's disposable task worktree is a linked git worktree, which is the shape `bin/fm-spawn.sh` always hands out, so it is out of scope.
A crew member using delegation tools inside its own task worktree is legitimate and stays allowed.
...
The hook scope is the shipped enforcement boundary, and the linked-worktree negative case proves the script itself does not block legitimate crew member delegation.
...
> [subagent-dispatch] the Number One primary dispatches through the fleet, not the harness's own
...
Every Number One guard's in-flight-work branch keys off `state/<id>.meta`, and only `bin/fm-spawn.sh` writes that record.
