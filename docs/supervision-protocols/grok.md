...
8. Waiting is silent.
9. Never use shell `&` for Number One supervision.
...
Headless `grok -p` may wait for background process exit but does not reliably surface full auto-wake model output; do not run the primary Number One as a one-shot headless process.
