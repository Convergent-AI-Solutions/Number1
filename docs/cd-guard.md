...
The primary Number One shell persists its working directory across tool calls.
A stray persistent top-level `cd projects/<clone>` therefore silently relocates the shell, so the next Number One-owned command - a backlog write, an `fm-*` lifecycle call, `tasks-axi` - runs inside a project clone instead of the home.
That has actually happened: a persistent top-level `cd` caused a Number One-owned backlog write to execute inside a project clone rather than the home.
...
## Scope: plain Number One checkouts only

The guard fires only in a plain Number One checkout where git-dir equals git-common-dir.
It is a silent no-op (exit 0, no output) everywhere else, so it never interferes with a crew member or scout that legitimately works inside its own project or Number One task worktree.
...
A crew member or scout task worktree - the shape `bin/fm-spawn.sh` always hands out - is a linked git worktree where the two differ, so the guard is inert there.
...
It therefore applies in a git-cloned second officer home where git-dir equals git-common-dir, but remains inert in a treehouse-leased second officer home that is itself a linked worktree.
Second officer child crew and scout worktrees are likewise inert under the linked-worktree test.
...
`bin/fm-cd-command-policy.mjs` imports the shell tokenizer and command-position analysis (`Lexer`, `splitProgram`, `commandPosition`) from `bin/fm-arm-command-policy.mjs`, the sole owner of Number One's shell classification.
...
| Codex | `.codex/hooks.json` PreToolUse hook that anchors from `pwd -P`, verifies the hook-loaded Number One root, and forwards the payload | Blocks on exit 2 and displays stderr. |
...
The suite also proves the end-to-end cwd-leak regression (a Number One-owned backlog write leaking into a project clone, then denied at the exact command), the checkout scoping (fires in a git-cloned second officer fixture, inert in a crew member/scout linked worktree, inert outside a Number One checkout, inert outside a git repo), the fail-open transport behavior, the prefilter fast path, the policy CLI output contract, and the per-harness wiring.
...
Each harness ran against a scratch primary-shaped Number One checkout: a plain git repo with `AGENTS.md`, `bin/` holding the real `fm-cd-pretool-check.sh`, `fm-cd-command-policy.mjs`, and `fm-arm-command-policy.mjs` plus a no-op dummy `fm-arm-pretool-check.sh`, a `projects/foo/` stand-in clone, and the tracked harness hook config.
