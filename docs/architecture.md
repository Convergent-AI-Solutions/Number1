...
How Number One works, in depth.

The [README](../README.md) carries the high-level diagram and a short synopsis.
This document expands every part of it.
Number One's always-loaded operating contract and routing index for conditional procedures is [`AGENTS.md`](../AGENTS.md); this is the human-facing companion.
...
Live or inconclusive liveness remains fail-open at that initial surface, and the second officer idle-endpoint exemption is unchanged.
...
For whole-fleet read-only review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v1` from the backlog, task metadata, current crew state, endpoint probes, PR/report pointers, scout reports, bounded current summaries from registered second officer homes, and second officer return-channel guidance.
`bin/fm-fleet-view.sh` renders that snapshot as Markdown for humans, while `bin/fm-bearings-snapshot.sh` provides the bounded bearings projection, so both views consume one structured contract instead of reparsing raw fleet files.
The script header owns the exact JSON schema.

### Registered second officer current state

A registered second officer's validated home is the authority for bearings current state because it owns the child metadata inventory, each child's current-state result, endpoint observations, backlog holds and dependencies, keyed unresolved decisions, and recent Done baseline.
The original cross-home projection instead treated the second officer agent as an ordinary parent task, so an idle second officer's `fm-crew-state` fallback selected the latest append-only parent status event even when structured state in the registered home contradicted it.
...
Generated second officer charters reject generic receipt or start acknowledgements, key only supervisor-actionable material phase reports, and close an opened phase with a same-key later state or `resolved` event, while the structured home remains authoritative even if that closure is missing.
...
Its `--restart` mode signals only the watcher recorded in the current home's `state/.watch.lock`, so restarting one home cannot kill sibling second officer watchers.
...
The guard covers the main primary and genuinely marked second officer homes, exempts child crew member/scout worktrees, is loop-safe per harness, and is documented in [turnend-guard.md](turnend-guard.md).
...
The daemon escalates captain-relevant events, plus a bounded recheck for a declared pause that remains idle, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so Number One can distinguish it structurally from real messages.
...
The runtime backend is the session-provider layer below Number One's scripts.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for second officer recovery, Zellij remains unverified, and Orca and cmux do not support second officer spawns.
...
Herdr's durable default container shape is workspace-per-home plus tab-per-task: the primary home uses workspace label `firstmate`, second officer homes use `2ndmate-<secondmate-id>`, and recovery/list-live scopes to the current `FM_HOME`'s workspace.
...
Orca is experimental and selected only explicitly: Orca owns both worktree and terminal lifecycle, records `orca_worktree_id=` and `terminal=`, and removes worktrees through `orca worktree rm` only after the usual Number One teardown checks pass.
...
Crew members never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, and cmux tasks, while Orca creates its own worktrees for `backend=orca`.
For ship and scout work, `fm-spawn.sh` refuses to launch unless the resolved task path is a real git worktree root that is distinct from the project primary checkout.

The Number One repo has one extra exposure because it can dispatch crew members to work on itself.
Its operating checkout (`FM_ROOT`) and the disposable crew member worktrees are all linked git worktrees of the same repository, so the valid discriminator is branch state, not whether the checkout is linked.
The primary checkout is healthy on its default branch, and linked worktrees or second officer homes are healthy at detached HEAD.
...
Ship briefs also tell the crew member to verify `pwd -P` and `git rev-parse --show-toplevel` before creating `fm/<id>`, then stop with a blocked status if it landed in the primary checkout.

## No-mistakes gate authority boundary

Number One's own no-mistakes gate runs agents inside a checkout that also contains the fleet-captain identity in `AGENTS.md`, so gate execution needs an authority boundary separate from ordinary crew member worktree isolation.
...
A normal primary checkout or crew member worktree has neither signal and remains unaffected.
...
Crew member and scout dispatch can stay on the static crew member harness resolved by `config/crew-harness`, or it can use local dispatch profiles in `config/crew-dispatch.json`.
The dispatch file is intentionally judgment-based: Number One reads the natural-language rules at intake, chooses the best matching rule, resolves profile arrays itself from current quota output under the `AGENTS.md` section 4 intake boundary and the `quota-array-dispatch` selection procedure, and passes only concrete `--harness`, `--model`, and `--effort` axes to `fm-spawn.sh`.
...
When the file exists, `fm-spawn.sh` refuses crew member and scout launches without an explicit harness, so `config/crew-harness` is only automatic when no dispatch profile file is active.
Second officer launches are exempt because they resolve the second officer harness and any optional second officer model or effort tokens instead.
...
## Optional second officers

`data/secondmates.md` records persistent second officers with natural-language scopes, project clone lists, and home paths.
...
For a domain whose subject is the Number One repo itself, a deliberate `--no-projects` seed creates a project-less home whose crews take pooled worktrees of that repo instead of separate clones.
The signal cannot be mixed with project names or omitted accidentally, and a populated home cannot be converted in place; the full seed contract is in [configuration.md](configuration.md#second-officer-routes-datasecondmatesmd).
On the herdr backend, a second officer launch lands in that second officer home's labeled workspace, and crew members spawned from that home land in the same workspace.
When seeded with `-`, the home is a durable treehouse lease under the second officer id, so it survives with no live process and is not recycled by later `treehouse get` or pruning.
Retirement or seed rollback returns the leased home; normal restart/recovery keeps it leased.
If returning the lease fails during teardown, Number One leaves the route and home intact instead of hiding a still-held lease.
...
The same project may appear in multiple second officer homes when their scopes differ, such as issue triage versus feature development.
Second officers are idle by default: after startup recovery reconciles only work already in their own home, an empty queue waits silently for routed tasks, and they never self-initiate surveys or audits.
When called with `FM_HOME=<this-firstmate-home>` or when `FM_HOME` is already set to the active Number One home, metadata-routed `fm-send.sh` requests to a live `kind=secondmate` use the live-charter-compatible `from-firstmate` carrier owned by `bin/fm-operational-input.sh`, so the second officer returns terse answers through status lines and detailed answers through docs plus status pointers instead of replying only in its own chat.
The parent guards every marked request against a missing correlated report without reading the second officer conversation; `bin/fm-pending-reply-lib.sh` owns the correlation, recovery, escalation, and retention contract.
Explicit backend-target sends and direct human typing stay unmarked, so captain intervention in a second officer pane remains conversational.
After seeding a second officer, `fm-backlog-handoff.sh` validates the fleet-specific handoff, then atomically delegates already-judged in-scope queued item moves to `tasks-axi mv` so the domain queue starts in the right place.
Idle second officer panes are healthy; teardown is explicit and refuses while the second officer home has in-flight work unless the captain has approved discard with `--force`.

Second officer homes converge conservatively to the primary's version and declared inherited local material at launch and during locked session start.
...
Second officer agents can run on a different verified harness than crew members.
`config/secondmate-harness` controls the primary's second officer launch harness and may also carry optional model and effort tokens as `<harness> [<model>] [<effort>]` on the first non-empty, non-comment line.
...
Those optional tokens are re-read on every second officer spawn or respawn and are overridden by explicit per-spawn `--model` or `--effort` flags.
...
`config/crew-harness` remains the crew member harness and is inherited into second officer homes.
`config/crew-dispatch.json` is inherited too; second officers use the same natural-language dispatch profiles when spawning their own crew members.
...
The `data/secondmates.md` line contract is owned by the [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md#routing-table), and the second officer environment variables are documented in [configuration.md](configuration.md).

## Project modes are explicit

`data/projects.md` records each project's delivery mode and optional `+yolo` autonomy flag.
`no-mistakes` projects run the full validation pipeline, `direct-PR` projects open PRs without that pipeline, and `local-only` projects stay local until Number One performs an approved fast-forward merge.
...
The Number One repo itself is the exception: its `.no-mistakes/` directory is local state, stays gitignored, and is rejected by CI if tracked.
...
A user enables it by putting `FMX_PAIRING_TOKEN` in the Number One home's gitignored `.env`; `FMX_RELAY_URL` is optional and defaults to `https://myfirstmate.io`.
That token is standing authorization for Number One to answer public mentions and act autonomously on normal reversible mention requests.
...
Newly offered mentions are stored as `state/x-inbox/<request_id>.json` and wake Number One once per retained request ID; the [X mode configuration reference](configuration.md#x-mode-env) owns the durable offer-marker and re-offer contract.
...
Actionable reversible requests run through Number One's normal intake, backlog, dispatch, investigation, or ship lifecycle.
...
Ship briefs prompt crew members to create or update those files through the normal delivery path; `data/projects.md` stays a thin private registry.
...
The full ownership rule - what is project-intrinsic versus fleet-private, and how Number One keeps the two apart without writing into project clones - is owned by [`AGENTS.md`](../AGENTS.md) (project and knowledge management).
...
Generalizable Number One knowledge goes to shared tracked docs through the normal PR pipeline; the firstmate-internal `/stow` deliberately never stores findings in either skill directory.
...
Wake-time refreshes can target a single clone by project name, so the primary home also catches up when a second officer reports a merge from its own home.
...
`/updatefirstmate` fast-forwards the running Number One repo and registered second officer homes from `origin`, then re-reads updated instructions and nudges updated second officers without touching project clones.
The update is fast-forward only: dirty, diverged, offline, and off-default targets are reported and left untouched.
The origin-based updater and the local second officer sync share the same guarded fast-forward helper; only the origin mode fetches.
The mechanics are owned by the `/updatefirstmate` skill and Number One's operating manual in [`AGENTS.md`](../AGENTS.md) (self-update).

## Restart-proof

Fleet state lives in each task's session-provider backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca when explicitly selected), no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent second officer homes.
...
At session start, confirmed-dead second officer agent endpoints are closed and relaunched through the same second officer spawn path, while ambiguous liveness reads are left untouched to avoid duplicate supervisors.
Use `/stow` before an intentional reset when the conversation may hold durable knowledge that has not yet been written to disk; after that, the next Number One session can reconcile and carry on.
