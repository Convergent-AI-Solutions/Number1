...
cmux is macOS-only, GUI-first, and unsuitable for a headless or SSH-only Number One session.
...
cmux defaults to a control mode that rejects external shells, while Number One always controls it from an external process.
Open Settings > Automation and choose a viable Socket Control Mode before the first cmux-backed spawn.

| Setting | Value | Number One support | Security boundary |
| --- | --- | --- | --- |
...
For Password mode, store the password as the first line of local gitignored `config/cmux-socket-password` or provide `CMUX_SOCKET_PASSWORD` in Number One's environment.
...
Select cmux with local `config/backend` containing `cmux`, `FM_BACKEND=cmux` for one launch, or an explicit request to Number One.
It can also be runtime auto-detected when Number One itself runs inside cmux.
...
The home label is `firstmate` or `2ndmate-<id>` plus a stable short hash of the resolved Number One root.
cmux does not enforce title uniqueness, so create, recovery, list, and cleanup paths all validate this scoped title.
Relocating the Number One installation changes the hash and leaves old titles unmatched, consistent with recorded worktree paths also becoming stale.
...
When the task is last in its window, Number One creates one unfocused unnamed sibling workspace in that same window, closes the task workspace, and leaves the window with cmux's fresh default workspace.
...
Number One does not attempt to close the macOS window because cmux's socket cannot close a window holding a live terminal.
...
- Second officer spawns are unsupported until a per-home lifecycle design is verified.
