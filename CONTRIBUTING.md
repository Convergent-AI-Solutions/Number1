...
There is no reliable way for `bin/fm-brief.sh`'s scaffold to detect that a task's repo is Number One itself, so Number One adds this skill's load line to firstmate-repo briefs by hand.
